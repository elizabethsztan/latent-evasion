...
from analysis._shared import (
    ...
    STEERED_POST_COLOR, apply_rcparams, cascade_pre_post,
    draw_single_probe_cascade_panel, load_probe,
)
...
def main():
    ...
    for ax, layer in zip(axes, plot_layers):
        ...
        mean_unsteered_score = float(np.mean(hf_post[:, layer, :] @ w + b))
        ...
        print(
            ...
            f"| probe acc = {acc:.3f} "
            f"| mean unsteered score = {mean_unsteered_score:.6f} "
